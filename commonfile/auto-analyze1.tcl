# Trajectory analysis in VMD
# Questions tugba.kucukkal@gallaudet.edu

# Usage: 1. Change the path and name for psf and dcd files
# Usage: 2. If running from local computer then, run the 'source auto-analyze.tcl'
# command in VMD Tk Console
# Usage: 3. If running from a Linux system such as a supercomputer, add the following 
# three lines into your bash script and then run your bash script: 
# module reset
# module load vmd
# vmd -dispdev text -e auto-analyze.tcl

# Load trajectory

mol new  4HCO_Thy9_6_QwikMD.psf type psf
mol addfile qwikmd_production_1.dcd type dcd first 0 last -1 step 1 waitfor all
...
hbonds -polar yes -sel1 $sel1 -frames all -dist 3.0 -ang 20 -plot no -writefile yes -outfile number.txt -type all -detailout details
.txt
